...
class DataSet():
    ...
    def scaler_data(self):
        ...
        ##### scale coods #####
        data_train_scaler.iloc[:, 1:3] = self.scaler_coods.fit_transform(
            self.train_data.values[:, 1:3])  # fit and transform
        data_test_scaler.iloc[:, 1:3] = self.scaler_coods.transform(self.test_data.values[:, 1:3])  # transform

        ##### scale auxiliary variables #####
        data_train_scaler.iloc[:, 3:-1] = self.scaler_features.fit_transform(
            self.train_data.values[:, 3:-1])  # fit and transform
        data_test_scaler.iloc[:, 3:-1] = self.scaler_features.transform(self.test_data.values[:, 3:-1])  # transform

        ##### scale target variable #####
        data_train_scaler.iloc[:, -1] = self.scaler_label.fit_transform(
            self.train_data.values[:, -1].reshape(-1, 1))  # fit and transform
        data_test_scaler.iloc[:, -1] = self.scaler_label.transform(
            self.test_data.values[:, -1].reshape(-1, 1))  # transform
